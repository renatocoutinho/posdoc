...
Neste projeto, estamos interessados no comportamento de sistemas no nível
...
Isso nos leva a considerar modelos dinâmicos cujas variáveis são populações,
representadas mais naturalmente por equações diferenciais ou a diferenças,
cujas previsões são obtidas usando técnicas analíticas e aproximações
...
já que a cada passo é necessário analisar as hipóteses e suas consequências,
assim como possíveis influências de simplificações necessárias a essa análise.
Após definidos os propósitos para o modelo que se pretende construir, procedemos
...
acrescentar uma taxa por mancha constante -- ``determinística'' --, como no
...
de sistemas de equações diferenciais, em que se busca as soluções
...
ou os autovalores de matrizes, são intratáveis usando apenas papel e caneta,
% ugh! ``manualmente'' também não é bom..
mas podem ser realizados com o auxílio de ferramentas computacionais, como a
a biblioteca para matemática simbólica SymPy \citep{sympy}.
...
um dele \citep{iserles2009}, que serão então integradas usando o mesmo método
de antes.

\subsubsection{Interpretação e síntese}

Que mecanismos permitem a persistência das espécies? Sob que condições há
maior diversidade de espécies, ou menores probabilidades de extinção? Essas e
outras questões de interesse ecológico só podem ser respondidas por modelos
quando é possível traçar uma analogia clara entre os processos introduzidos no
modelo e seus parâmetros. Esta última etapa consiste em conectar as hipóteses
assumidas no início e conectá-las às conclusões obtidas, buscando compreender
o mecanismo que leva das primeiras às últimas à luz do modelo desenvolvido. 

% a terminar..

%Nes avaliando a , até alcançar modelos que sejam informativos,
